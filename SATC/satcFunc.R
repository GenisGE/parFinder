...
\t\t--lengthWeight:\t Weight normalizing scaffolds by their length\n 
...
readArgs <- function(args){
    ...
    for(i in seq(1, length(args), 2)){
        ...
        } else if(args[i] == "--lengthWeight"){ pars$lengthWeight <- as.logical(args[i+1])
...
satc <- function(SPECIES,IDXFILE,OUTFOLD,minLength=1e5,M=5, weight=TRUE, K=2, model="gaussian", normScaffolds=NULL, useMedian=FALSE){ 
    ...
    ## identify sex
    sex <- sexDetermine(dat=rFilt, K=K, weight=weight, model=model, lengthWeight=lengthWeight) 
